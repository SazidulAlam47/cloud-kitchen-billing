...
interface CorporateFormProps {
  ...
  onSubmit: (data: CorporateBill) => void;
  isEditing?: boolean;
}

export function CorporateForm({ initialData, onSubmit, isEditing = false }: CorporateFormProps) {
  const navigate = useNavigate();

  const { register, control, handleSubmit, setValue, formState: { errors } } = useForm<CorporateBill>({
  ...
  const onFormSubmit = (data: CorporateBill) => {
    onSubmit(data);
  };
                 ...
                 return (
                  <div key={field.id} className="grid grid-cols-12 gap-4 items-center bg-gray-50/50 p-3 rounded-lg border border-gray-100 group hover:border-gray-300 transition-all">
                    <div className="col-span-3">
                      <Input
                        type="date"
                        {...register(`items.${index}.date` as const, { required: true })}
                        className="bg-white"
                      />
                    </div>
                    <div className="col-span-3">
                      <Input
                        ...
                        {...register(`items.${index}.packageType` as const, { required: true })}
                        className="bg-white"
                      />
                    </div>
